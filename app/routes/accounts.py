...
async def page_edit(account_id: uuid.UUID, request: Request, templates: Jinja2Templates = TemplatesDep):
    return templates.TemplateResponse(
        request,
        "entities/form.html",
...
async def api_list(client_id: uuid.UUID | None = None, session: AsyncSession = SessionDep):
...
async def api_get(account_id: uuid.UUID, session: AsyncSession = SessionDep):
...
async def api_create(data: AccountCreate, session: AsyncSession = SessionDep):
    item = await AccountService(session).create(data)
    return AccountRead.model_validate(item, from_attributes=True)


@router.put("/api/accounts/{account_id}", response_model=AccountRead)
async def api_update(account_id: uuid.UUID, data: AccountUpdate, session: AsyncSession = SessionDep):
    item = await AccountService(session).update(account_id, data)
    return AccountRead.model_validate(item, from_attributes=True)


@router.delete("/api/accounts/{account_id}", status_code=204)
async def api_delete(account_id: uuid.UUID, session: AsyncSession = SessionDep):
